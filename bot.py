...
import sys
import datetime
import random  # Added for audio/gif selection
...
from telethon.tl.types import (
    ...
    ChannelParticipantsAdmins, DocumentAttributeAudio
)
...
# Import farewell constants
try:
    from assets.farewell.constants import FAREWELL_GIFS, FAREWELL_AUDIO, FAREWELL_AUDIO_FILES, CREATOR_NAME, CREATOR_USERNAME, CREATOR_LINK
    from assets.stickers.constants import FAREWELL_STICKER_ID, FAREWELL_STICKERS
except ImportError:
    logger.warning("Farewell constants not found, using default values")
    ...
    FAREWELL_GIFS = ["https://giphy.com/gifs/ySEd2yk70L4Fz70tyn"]  # Default GIF
    FAREWELL_AUDIO = "assets/farewell/bye.m4a"  # Default audio
    FAREWELL_AUDIO_FILES = [FAREWELL_AUDIO]  # Default audio files list with single entry
    CREATOR_NAME = "ªᴹᴷᵁˢᴴ"
...
# Version information 
BOT_VERSION = "1.3.1"  
VERSION_DATE = "2025-05-04"
LAST_UPDATE = "Improved hyperlink handling in media captions during edits"
...
# Function to get the current state of deletion synchronization
def get_sync_deletions():
    """Get the current state of deletion synchronization from BOT_CONFIG"""
    from config import BOT_CONFIG  # Import here to get fresh value
    sync_value = BOT_CONFIG.get("sync_deletions", False)
    logger.info(f"Sync deletions is currently: {sync_value}")
    return sync_value


# Enable developer mode for additional debugging information in UI
DEVELOPER_MODE = BOT_CONFIG.get("developer_mode", True)  # Default to True for better error messages
# Make sure we save the developer mode to the config
BOT_CONFIG["developer_mode"] = DEVELOPER_MODE

# Make sure the sync_deletions setting is saved in BOT_CONFIG
def save_reposting_state():
    ...
    BOT_CONFIG["sync_deletions"] = True  # Always enable sync_deletions
    save_bot_config()
...
# Key fix: Make sure destinations is always a list
if not isinstance(active_channels["destinations"], list):
    logger.warning(f"Destinations is not a list type: {type(active_channels['destinations']).__name__}, fixing...")
    if active_channels["destinations"] is None:
        active_channels["destinations"] = []
    else:
        # Convert to list if it's not already
        active_channels["destinations"] = [active_channels["destinations"]]
...
# Log the state of destinations at startup for debugging
logger.info(f"Destination channels at startup: {active_channels['destinations']} (type: {type(active_channels['destinations']).__name__})")
...
# This function is now defined above to include sync_deletions
# Using the unified function at the top of the file that handles both reposting_active and sync_deletions
...
if "reposting_active" in BOT_CONFIG:
    ...
    # Note: We can't use await here since we're in the module level code, not in an async function.
    # The function is designed to handle this by directly calling save_bot_config()
    try:
...
async def normalize_channel_id(channel_input: Union[int, str]) -> Union[int, str]:
    ...
    try:
        # If it's None, just return None - this is important for clearing channels
        if channel_input is None:
            return None
            
        # If it's already an integer, just return it as is
        ...
        if isinstance(channel_input, str):
            ...
            if channel_input.lower().strip() in ('none', 'null'):
                return None
...
async def save_config():
    ...
    # Make sure destinations is a list before saving
    if not isinstance(active_channels["destinations"], list):
        logger.warning(f"Destinations is not a list when saving: {type(active_channels['destinations']).__name__}")
        if active_channels["destinations"] is None:
            active_channels["destinations"] = []
        else:
            active_channels["destinations"] = [active_channels["destinations"]]
    ...
    # Ensure we're not saving None values in lists
    clean_source = [ch for ch in active_channels["source"] if ch is not None]
    clean_destinations = [ch for ch in active_channels["destinations"] if ch is not None]
    
    # Remove duplicate destinations by comparing string representations
    unique_destinations = []
    seen_destination_strs = set()
    for ch in clean_destinations:
        ch_str = str(ch)
        if ch_str not in seen_destination_strs:
            seen_destination_strs.add(ch_str)
            unique_destinations.append(ch)
    
    # Create clean config with the filtered data
    config = {
        "source_channels": clean_source,
        "destination_channel": active_channels["destination"],
        "destination_channels": unique_destinations
    }
    
    # Log what we're about to save with detailed type information
    logger.info(f"Saving configuration:")
    logger.info(f"Source channels: {clean_source} (types: {[type(ch).__name__ for ch in clean_source]})")
    logger.info(f"Primary destination: {active_channels['destination']} (type: {type(active_channels['destination']).__name__})")
    logger.info(f"Destinations: {unique_destinations} (types: {[type(ch).__name__ for ch in unique_destinations]})")
    
    # Convert to JSON
    config_json = json.dumps(config)
    
    # Validate the JSON by parsing it back to ensure it's correct
    try:
        test_parse = json.loads(config_json)
        if not isinstance(test_parse.get("destination_channels"), list):
            logger.error(f"JSON serialization issue - destinations not a list after parsing: {type(test_parse.get('destination_channels')).__name__}")
            # Force it to be a list
            test_parse["destination_channels"] = list(unique_destinations)
            config_json = json.dumps(test_parse)
    except Exception as e:
    ...
    try:
        ...
        if "CHANNEL_CONFIG=" in env_content:
            # Replace existing value with clean quotes
        ...
        logger.info(f"✅ Successfully updated channel configuration in .env file")
    except Exception as e:
        error_msg = str(e)
        logger.error(f"❌ Failed to update .env file with channel configuration: {error_msg}")
        logger.info(f"Updated channel configuration in memory only: {config_json}")
    
    # Force reload the configuration from memory
...
async def detect_markdown_links(text):
    """
    Detect both markdown style links [text](url) and plain URLs in text
    ...
    # URL pattern for detecting plain URLs that aren't in markdown format
    # This matches most common URL formats including http://, https://, and t.me/ links
    # Expanded to catch more URL variants while avoiding false positives
    url_pattern = r'(?<![\[\(])((?:https?://|www\.|t(?:elegram)?\.me/)[^\s\[\]\(\)\"\'<>]+)(?![\]\)])'
    
    # Find all markdown links
    markdown_links = list(re.finditer(markdown_pattern, text))
    
    # Find all plain URLs
    url_links = list(re.finditer(url_pattern, text))
    
    if not markdown_links and not url_links:
        return text, []
    ...
    # Process plain URL links
    for match in url_links:
        # The URL is the entire match
        url = match.group(1)
        # For plain URLs, the link text is the same as the URL
        link_text = url
        
        # Make sure the link has a proper protocol
        if url.startswith('www.'):
            url = 'https://' + url
        elif url.startswith('t.me/') or url.startswith('telegram.me/'):
            url = 'https://' + url
            logger.info(f"Fixed Telegram URL to include protocol: {url}")
            
        logger.info(f"Extracted plain URL: '{url}'")
        
        links_to_process.append({
            'start': match.start(1),  # Using group 1 index to get just the URL part
            'end': match.end(1),
            'text': link_text,
            'url': url,
    ...
    # Sort them from end to start to avoid offset issues when modifying text
    ...
    for link in links_to_process:
        ...
        # For markdown links, replace with just the text
        # For plain URLs, leave the text as is (no replacement needed)
        if link['type'] == 'markdown':
            modified_text = modified_text[:link['start']] + link['text'] + modified_text[link['end']:]
    
    # Log the transformation for debugging
    if text != modified_text:
        logger.info(f"Link transformation: '{text}' → '{modified_text}'")
    logger.info(f"Generated {len(entities)} entities from links in text")
    ...
    for i, entity in enumerate(entities):
        if links_to_process[len(links_to_process) - 1 - i]['type'] == 'markdown':
            entity['offset'] += offset_adjustment
            # Calculate adjustment for next entities (only for markdown links that we modified)
...
# Message mapping storage with improved memory optimization
# Format: {
#    (source_channel_id, source_message_id): {
#        "destinations": {dest_channel: dest_msg_id},
#        "timestamp": datetime object,
#        "last_accessed": datetime object
#    }
# }
message_mapping = {}

# Configuration for memory optimization
MAX_RECENT_MESSAGES = 50  # Maximum number of recent messages to store
MAX_CACHE_AGE_HOURS = 48  # Messages older than this will be expired (in hours)
CACHE_CLEANUP_INTERVAL = 100  # Run cleanup every N message operations
FREQUENTLY_ACCESSED_BONUS_HOURS = 12  # Keep frequently accessed messages longer
...
memory_stats = {
    ...
    "expired_by_count": 0,
    "expired_by_time": 0,
    "last_cleanup": datetime.datetime.now(timezone.utc)
}

# List to track the order of message mappings (for cleaning up oldest entries)
message_mapping_order = []

# Counter for tracking operations since last cleanup
op_counter = 0

# Function to add a message mapping to the recent messages cache (limited to 50 messages)
async def add_message_mapping(source_channel_id, source_message_id, dest_channel, dest_msg_id):
    """Add a message mapping with optimized storage
    
    This function stores message mappings with:
    - Count-based limits (50 most recent messages)
    - Time-based expiration (messages older than 48 hours)
    - Access frequency tracking (frequently accessed messages stay longer)
    
    This approach balances memory efficiency with practical edit synchronization.
    """
    global op_counter, memory_stats
    current_time = datetime.datetime.now(timezone.utc)
    key = (source_channel_id, source_message_id)
    
    # Increment operation counter for cleanup scheduling
    op_counter += 1
    memory_stats["total_messages_tracked"] += 1
    
    # Check if mapping exists
    if key not in message_mapping:
        # Create new mapping entry with timestamp and structure
        message_mapping[key] = {
            "destinations": {},
        ...
        memory_stats["cache_misses"] += 1
        logger.info(f"Added new mapping for source ({source_channel_id}, {source_message_id})")
    else:
        # Update last_accessed time for frequently used messages
        message_mapping[key]["last_accessed"] = current_time
        memory_stats["cache_hits"] += 1
    
    # Add or update the destination mapping
    previous_msg_id = message_mapping[key]["destinations"].get(dest_channel)
    message_mapping[key]["destinations"][dest_channel] = dest_msg_id
    ...
    # Run time-based and count-based cleanups periodically
    if op_counter >= CACHE_CLEANUP_INTERVAL:
        await cleanup_message_cache()
        op_counter = 0

async def cleanup_message_cache():
    """Clean up the message cache based on age and count limits"""
    global memory_stats
    current_time = datetime.datetime.now(timezone.utc)
    memory_stats["cleanup_runs"] += 1
    memory_stats["last_cleanup"] = current_time
    
    # Enforce count-based limits (keep the most recent MAX_RECENT_MESSAGES)
    count_expired = 0
    while len(message_mapping_order) > MAX_RECENT_MESSAGES:
        oldest_key = message_mapping_order.pop(0)  # Remove oldest entry
        if oldest_key in message_mapping:
            message_mapping.pop(oldest_key, None)
            count_expired += 1
    
    if count_expired > 0:
        memory_stats["expired_by_count"] += count_expired
        logger.info(f"Count-based cleanup: Removed {count_expired} oldest mappings")
    
    # Enforce time-based limits
    time_expired = 0
    expiration_cutoff = current_time - datetime.timedelta(hours=MAX_CACHE_AGE_HOURS)
    
    # Messages accessed recently get a time bonus
    recent_access_cutoff = current_time - datetime.timedelta(hours=FREQUENTLY_ACCESSED_BONUS_HOURS)
    
    expired_keys = []
    for key in list(message_mapping.keys()):
        mapping_data = message_mapping[key]
        
        # Skip frequently accessed messages
        if mapping_data["last_accessed"] > recent_access_cutoff:
            continue
            
        # Remove messages older than the expiration cutoff
        if mapping_data["timestamp"] < expiration_cutoff:
            expired_keys.append(key)
    
    # Remove the expired keys
    for key in expired_keys:
        if key in message_mapping:
            message_mapping.pop(key)
            if key in message_mapping_order:
                message_mapping_order.remove(key)
            time_expired += 1
    
    if time_expired > 0:
        memory_stats["expired_by_time"] += time_expired
        logger.info(f"Time-based cleanup: Removed {time_expired} expired mappings")
    
    # Log memory usage statistics
    total_mappings = len(message_mapping)
    logger.info(f"Memory usage: {total_mappings} active mappings")
    logger.info(f"Memory stats: {memory_stats}")
    
    return count_expired + time_expired
...
async def handle_edited_message(event):
    ...
    This function now features improved handling of media edits, particularly with hyperlinks in captions:
    1. First attempts to edit the caption in place when only text is changed
    2. Preserves and properly formats hyperlinks in media captions
    3. Only falls back to delete-and-repost when caption editing fails
    
    This approach minimizes duplicate posts and improves synchronization between channels.
    """
    # Log the edit event type for debugging
    if hasattr(event, 'message') and hasattr(event.message, 'media'):
        has_media = event.message.media is not None
        logger.info(f"Edited message with media={has_media} received")
        
        # Log if there's a caption with potential hyperlinks
        if has_media and hasattr(event.message, 'message'):
            caption = event.message.message
            has_links = '[' in caption and '](' in caption if caption else False
            logger.info(f"Media caption contains potential hyperlinks: {has_links}")
...
async def handle_deleted_message(event):
    ...
    # Check if deletion synchronization is enabled by calling our function
    if not get_sync_deletions():
    ...
    try:
        ...
        for deleted_id in deleted_ids:
            ...
            if key in message_mapping:
                ...
                if "destinations" in mapping_data:  # New format
                    mapping_data["last_accessed"] = datetime.datetime.now(timezone.utc)
                    destinations_dict = mapping_data["destinations"]
                else:  # Old format - convert to new format
                    destinations_dict = mapping_data  # The old format is just the destinations dict
                    ...
                    # Update mapping_data reference to point to the new structure
                    mapping_data = message_mapping[key]
                    logger.info(f"Upgraded old format message mapping to new format for deleted message key {key}")
                
                for dest_channel, dest_msg_id in destinations_dict.items():
                    try:
                ...
                logger.info(f"No mapping found for deleted message {deleted_id} (message not in the last {MAX_RECENT_MESSAGES} messages)")
    except Exception as e:
    ...
    try:
        ...
        if is_edit and source_channel_id and source_message_id:
            ...
            if key in message_mapping:
                ...
                # Update memory statistics
                memory_stats["cache_hits"] += 1
                message_mapping[key]["last_accessed"] = datetime.datetime.now(timezone.utc)
                
                # For each destination where we previously sent this message
                if "destinations" in message_mapping[key]:  # Use the new structure
                    destinations_dict = message_mapping[key]["destinations"]
                else:  # For backward compatibility with older entries
                    destinations_dict = message_mapping[key]
                    # Convert old format to new format on access
                    message_mapping[key] = {
                        "destinations": destinations_dict,
                        "timestamp": datetime.datetime.now(timezone.utc),
                        "last_accessed": datetime.datetime.now(timezone.utc)
                    }
                    logger.info(f"Upgraded old format message mapping to new format for key {key}")
                
                # Now iterate through the destinations
                for dest_channel, dest_msg_id in destinations_dict.items():
                    ...
                    try:
                        ...
                        if msg_data["has_media"]:
                            # For media edits, we need special handling to properly handle hyperlinks
                            logger.info(f"Media message edit detected. Attempting to handle properly.")
                            
                            # First try to edit the caption in place, as media might not have changed
                            try:
                                # Process caption for hyperlinks if applicable
                                caption_html = None
                                
                                if msg_data["media_data"]["caption"]:
                                    caption_text = msg_data["media_data"]["caption"]
                                    
                                    # Use our enhanced detect_markdown_links that now handles both 
                                    # markdown-style links and regular URLs
                                    processed_text, detected_links = await detect_markdown_links(caption_text)
                                    
                                    if detected_links:
                                        logger.info(f"Found {len(detected_links)} hyperlinks in edited caption")
                                        logger.info(f"Edited caption link details: {[{'url': link['url'], 'text': processed_text[link['offset']:link['offset']+link['length']]} for link in detected_links]}")
                                        
                                        # Create message entities from the detected links
                                        caption_entities = []
                                        for e in detected_links:
                                            # Ensure URL has protocol
                                            link_url = e['url']
                                            if link_url.startswith('t.me/') or link_url.startswith('telegram.me/'):
                                                link_url = 'https://' + link_url
                                                logger.info(f"Fixed URL in edited caption to include protocol: {link_url}")
                                        ...
                                        modified_text, processed_entities = await find_replace_channel_tags(processed_text, caption_entities)
                                        ...
                                        for entity_dict in sorted_entities:
                                            if entity_dict['type'] == 'MessageEntityTextUrl':
                                                ...
                                                if start >= len(modified_text):
                                                    logger.warning(f"Entity start {start} is beyond text length {len(modified_text)}")
                                                    continue
                                                    
                                                if end > len(modified_text):
                                                    logger.warning(f"Entity end {end} is beyond text length {len(modified_text)}")
                                                    end = len(modified_text)
                                                
                                                # Add text before this entity
                                                if start > last_end and last_end < len(modified_text):
                                        ...
                                        logger.info(f"Formatted HTML caption for edit: {caption_html}")
                                
                                # Try to directly edit the caption without deleting the media
                                caption_to_use = caption_html if caption_html else msg_data["media_data"]["caption"]
                                ...
                                logger.info(f"Successfully edited media caption for message {dest_msg_id} in {dest_channel}")
                                
                                # Mark this destination as handled so we don't repost
                                sent_destinations[dest_channel] = dest_msg_id
                                
                            except Exception as e:
                                logger.error(f"Couldn't edit caption for media message {dest_msg_id}: {e}")
                                logger.info(f"Will delete and repost the media instead")
                                
                                # Fallback: delete and repost if caption edit fails
                                try:
                                    # Delete the old message to avoid having two versions
                                    await user_client.delete_messages(dest_channel, dest_msg_id)
                                    logger.info(f"Deleted old media message {dest_msg_id} in channel {dest_channel} to avoid duplicates")
                                    
                                    # Note: We explicitly do NOT add to sent_destinations here
                                    # This ensures the reposting logic below will create a new post
                                    # and properly record the new message ID in the mapping
                                except Exception as e:
                                    logger.error(f"Error deleting old media message {dest_msg_id}: {e}")
                                    # Even if deletion fails, we'll still try to create a new post below
                            
                            # If we didn't successfully edit the caption, the regular flow below will handle reposting the updated media
                        else:
                ...
                logger.info(f"No mapping found for edited message (message not in the last {MAX_RECENT_MESSAGES} messages)")
        ...
        if msg_data["has_media"]:
            ...
            if msg_data["media_data"]["caption"]:
                # Process any caption text, regardless of format
                caption_text = msg_data["media_data"]["caption"]
                logger.info(f"Processing caption: {caption_text[:50]}...")
                
                # We'll use our enhanced detect_markdown_links function which now handles both 
                # markdown-style links [text](url) and regular URLs like https://example.com
                processed_text, detected_links = await detect_markdown_links(caption_text)
                
                if detected_links:
                    logger.info(f"Found {len(detected_links)} hyperlinks in caption (both markdown & URLs)")
                    logger.info(f"Caption link details: {[{'url': link['url'], 'text': processed_text[link['offset']:link['offset']+link['length']]} for link in detected_links]}")
                    ...
                    for e in detected_links:
                        ...
                        if link_url.startswith('t.me/') or link_url.startswith('telegram.me/'):
                            link_url = 'https://' + link_url
                            logger.info(f"Fixed URL in caption to include protocol: {link_url}")
                    ...
                    # Extra logging to check if HTML is being generated properly
                    if "http" in caption_html:
                        logger.info("HTML caption contains hyperlinks - should be displayed as clickable links")
                        logger.info(f"HTML tags in caption: {len(re.findall(r'<a href=', caption_html))}")
                else:
                    # No hyperlinks found, still process for channel tags
                    modified_text, _ = await find_replace_channel_tags(caption_text)
                    caption_html = None  # No HTML formatting needed
                    # Update the caption in the media data
                    msg_data["media_data"]["caption"] = modified_text
            ...
            for dest_channel in destinations:
                try:
                    ...
                    # Fallback - try sending without special attributes but still with HTML
                    try:
                        # Always make sure to use HTML parsing mode for captions with links
                        caption_to_use = caption_html if caption_html else msg_data["media_data"]["caption"]
                        logger.info(f"Using fallback method with HTML caption: {caption_to_use[:50]}...")
                        
                        # Explicitly check if we have hyperlinks but no HTML formatting
                        if caption_to_use and "http" in caption_to_use and "<a href=" not in caption_to_use:
                            logger.info("Caption contains URLs but no HTML formatting - attempting to create HTML links")
                            try:
                                # Try to convert URLs to HTML links as a last resort
                                processed_text, detected_links = await detect_markdown_links(caption_to_use)
                                if detected_links:
                                    logger.info(f"Found {len(detected_links)} links in fallback caption")
                                    # Create HTML version
                                    parts = []
                                    last_end = 0
                                    
                                    # Sort links by position
                                    sorted_links = sorted(detected_links, key=lambda x: x['offset'])
                                    ...
                                    for link in sorted_links:
                                        ...
                                        if start > last_end:
                                            parts.append(processed_text[last_end:start])
                                        
                                        # Add HTML link
                                        link_text = processed_text[start:end]
                                        parts.append(f'<a href="{link["url"]}">{link_text}</a>')
                                    ...
                                    if last_end < len(processed_text):
                                        parts.append(processed_text[last_end:])
                                    
                                    # Replace caption with HTML version
                                    caption_to_use = ''.join(parts)
                                    logger.info(f"Created HTML caption in fallback: {caption_to_use[:50]}...")
                            except Exception as html_err:
                                logger.error(f"Error creating HTML caption in fallback: {str(html_err)}")
                        ...
                        logger.info(f"Sent media using fallback method to {dest_channel} (with HTML caption)")
                        
                        # Store message mapping for edit synchronization
                        if not is_edit and source_channel_id and source_message_id and dest_message:
                            dest_msg_id = dest_message.id
                        ...
                        # Last resort - try as document but still preserve HTML in caption
                        try:
                            # Use the already processed caption_to_use which should have HTML formatting
                            # If we got to this point, caption_to_use is already defined from earlier
                            logger.info(f"Last resort: sending as document with HTML caption")
                            dest_message = await user_client.send_file(
                                dest_channel,
                                msg_data["file_path"],
                                caption=caption_to_use,  # Use the already processed caption
                                parse_mode='html',
                                force_document=True
                            )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    settings_buttons = [
        ...
        [InlineKeyboardButton("🚮 Nucl3ar Option", callback_data="reset_all_channels")]
    ]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "add_source":
        ...
        for channel in active_channels["source"]:
            ...
            # Always use string version of channel ID for callback_data to avoid format issues
            channel_str = str(channel).strip()
            safe_callback = f"remove_{channel_str}"
            logger.info(f"Creating source removal callback data: {safe_callback}")
            logger.info(f"For channel: {channel} (type: {type(channel).__name__})")
            keyboard.append([InlineKeyboardButton(f"🗑️ Remove: {display_name}", callback_data=safe_callback)])
    ...
    elif query.data.startswith("remove_") and not (
        query.data.startswith("remove_dest_") or 
        query.data.startswith("remove_dest_idx_") or 
        query.data.startswith("force_remove_dest_") or
        query.data == "remove_multi_destination" or
        query.data == "direct_remove_2502786837"
    ):
        # This handler is specifically for source channel removal
        # We need to carefully exclude all destination-related removal patterns
        try:
            # Log detailed information for debugging
            logger.info(f"Processing source channel removal: {query.data}")
            
            # Extract the channel ID
            if len(query.data.split("_")) > 1:
                channel_id_str = query.data.split("_")[1]
                logger.info(f"Extracted channel ID string: {channel_id_str}")
                
                # Improved channel ID handling that works with both integer and string IDs
                # First, try to find the exact channel in the sources
                found = False
                removed_channel = None
                
                # Log the source channels for debugging
                logger.info(f"Source channels before removal: {active_channels['source']}")
                logger.info(f"Types: {[type(ch).__name__ for ch in active_channels['source']]}")
                
                # Strategy 1: Try direct string match if it's a string ID
                if not found and channel_id_str in active_channels["source"]:
                    active_channels["source"].remove(channel_id_str)
                    removed_channel = channel_id_str
                    found = True
                    logger.info(f"✓ Removed channel by direct string match: {removed_channel}")
                
                # Strategy 2: Try integer conversion for numeric channel IDs
                if not found and channel_id_str.lstrip('-').isdigit():
                    try:
                        channel_id_int = int(channel_id_str)
                        logger.info(f"Converted to integer: {channel_id_int}")
                        
                        if channel_id_int in active_channels["source"]:
                            active_channels["source"].remove(channel_id_int)
                            removed_channel = channel_id_int
                            found = True
                            logger.info(f"✓ Removed channel by integer value: {removed_channel}")
                    except ValueError as e:
                        logger.warning(f"Integer conversion failed: {str(e)}")
                
                # Strategy 3: Compare string representations for type mismatches
                if not found:
                    for idx, channel in enumerate(active_channels["source"]):
                        if str(channel) == channel_id_str:
                            removed_channel = active_channels["source"].pop(idx)
                            found = True
                            logger.info(f"✓ Removed channel by string comparison at index {idx}: {removed_channel}")
                            break
                
                # Save configuration and show appropriate message
                if found and removed_channel is not None:
                    # Save configuration first
                    await save_config()
                    
                    # CRITICAL FIX: Re-register event handlers to ensure removed source is no longer monitored
                    # This is crucial to ensure the removed source channel doesn't continue triggering events
                    if user_client and user_client.is_connected():
                        logger.info("Updating Telegram client event handlers to reflect removed source channel")
                        
                        # First remove all existing event handlers for message events
                        if hasattr(user_client, '_event_builders'):
                            builders = list(user_client._event_builders)
                            for builder in builders:
                                if isinstance(builder[0], (events.NewMessage, events.MessageEdited, events.MessageDeleted)):
                                    logger.info(f"Removing old event handler for {builder[0].__class__.__name__}")
                                    user_client.remove_event_handler(builder[1], builder[0])
                        
                        # Re-register event handlers with the updated source list
                        if active_channels["source"]:
                            logger.info(f"Registering updated event handlers for source channels: {active_channels['source']}")
                            ...
                            # Add handler for deleted messages (if sync_deletions is enabled)
                            if BOT_CONFIG.get("sync_deletions", False):
                                user_client.add_event_handler(
                                    handle_deleted_message,
                                    events.MessageDeleted(chats=active_channels["source"])
                                )
                                
                            logger.info("Event handlers successfully updated")
                        else:
                            logger.info("No source channels left, not registering any event handlers")
                    else:
                        logger.warning("User client not available, couldn't update event handlers")
                    
                    await query.edit_message_text(
                        f"✅ Successfully removed channel {removed_channel} from sources.\n\nEvent handlers have been updated to stop monitoring this channel.",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back to Menu", callback_data="back_to_menu")]])
                    )
                else:
                    # Detailed debug info for troubleshooting
                    debug_info = f"\nChannel ID: {channel_id_str}"
                    debug_info += f"\nSource channels: {active_channels['source']}"
                    debug_info += f"\nTypes: {[type(ch).__name__ for ch in active_channels['source']]}"
                    
                    logger.warning(f"Channel not found in sources: {debug_info}")
                    await query.edit_message_text(
                        "❌ Channel not found in source channels.",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back to Menu", callback_data="back_to_menu")]])
                    )
            else:
                # Handle case where there's no channel ID in the callback data
                logger.warning(f"No channel ID found in callback data: {query.data}")
                await query.edit_message_text(
                    "❌ Missing channel ID in removal request.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back to Menu", callback_data="back_to_menu")]])
                )
        except Exception as e:
            # Handle any other exceptions
            logger.error(f"Error in source channel removal: {str(e)}")
            await query.edit_message_text(
                f"❌ Error removing source channel: {str(e)}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back to Menu", callback_data="back_to_menu")]])
            )
        ...
        logger.info(f"Active channels data: {active_channels}")
        logger.info(f"Destinations type: {type(active_channels['destinations'])}")
        logger.info(f"Destinations: {active_channels['destinations']}")
        
        # Force destinations to be a list if it somehow became something else
        if not isinstance(active_channels["destinations"], list):
            logger.warning(f"Destinations is not a list, converting from {type(active_channels['destinations']).__name__}")
            # Try to convert to a list if it's a string or other type
            try:
                if active_channels["destinations"] is None:
                    active_channels["destinations"] = []
                elif isinstance(active_channels["destinations"], str):
                    # Try to parse as JSON
                    try:
                        parsed = json.loads(active_channels["destinations"])
                        if isinstance(parsed, list):
                            active_channels["destinations"] = parsed
                        else:
                            active_channels["destinations"] = [active_channels["destinations"]]
                    except:
                        active_channels["destinations"] = [active_channels["destinations"]]
                else:
                    # Try to convert to a list in other cases
                    active_channels["destinations"] = [active_channels["destinations"]]
            except Exception as e:
                logger.error(f"Error converting destinations to list: {str(e)}")
                active_channels["destinations"] = []
        ...
        if active_channels["destinations"]:
            keyboard.append([InlineKeyboardButton("➖ Remove Destination", callback_data="remove_multi_destination")])
            
            # Add nuclear option to clear all destinations
            keyboard.append([InlineKeyboardButton("🧨 Reset All Destinations", callback_data="reset_all_destinations")])
    ...
    elif query.data == "reset_all_destinations":
        # This is the "nuclear option" that completely resets all destinations
        # Show a confirmation dialog first
        text = "⚠️ WARNING: RESET ALL DESTINATIONS ⚠️\n\n" \
               "You are about to remove ALL destination channels from your configuration.\n\n" \
               "This will:\n" \
               "• Delete all destination channels\n" \
               "• Clear message mappings\n" \
               "• Reset any channel permissions\n\n" \
               "Are you sure you want to continue?"
               
        keyboard = [
            [InlineKeyboardButton("✅ Yes, Reset Everything", callback_data="confirm_reset_all_destinations")],
            [InlineKeyboardButton("❌ No, Cancel", callback_data="manage_destinations")]
        ]
    ...
    elif query.data == "confirm_reset_all_destinations":
        # User confirmed the reset, so clear all destinations
        # Track what we had before for the log message
        previous_destinations = active_channels["destinations"].copy() if isinstance(active_channels["destinations"], list) else []
        previous_destination = active_channels["destination"]
        ...
        # Clear message mappings
        message_mapping.clear()  # Clear the dictionary instead of reassigning
        
        # Log what happened
        logger.info(f"Reset all destinations. Previous destinations: {previous_destinations}")
        logger.info(f"Previous primary destination: {previous_destination}")
        
        # Show confirmation message
        text = "✅ All Destination Channels Have Been Reset\n\n" \
               "Your configuration has been updated and all destination channels have been removed.\n\n" \
               "You can now add new destination channels to start fresh."
        
        await edit_message_smartly(
            query.message,
            text,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back to Menu", callback_data="manage_destinations")]])
        )
    ...
    elif query.data == "remove_multi_destination":
        ...
        # Print the destinations list for debugging
        logger.info(f"Creating destination removal menu. Current destinations: {active_channels['destinations']}")
        logger.info(f"Types: {[type(d).__name__ for d in active_channels['destinations']]}")
        
        keyboard = []
        # Special method to handle channel IDs more carefully
        for idx, channel in enumerate(active_channels["destinations"]):
            # Get channel info
            info = await get_entity_info(user_client, channel)
            display_name = info.get("title", str(channel)) if info else str(channel)
            
            # Create a special key for this specific channel using a mix of type and index
            # This avoids relying on perfect string conversion of the channel ID
            channel_str = str(channel).strip()
            
            # Add the index to make callback data unique even if channel IDs look similar
            safe_callback = f"remove_dest_idx_{idx}_{channel_str}"
            
            # Log what we're doing
            logger.info(f"Creating removal callback for index {idx}: {channel} ({type(channel).__name__})")
            logger.info(f"Callback data: {safe_callback}")
            
            keyboard.append([InlineKeyboardButton(f"🗑️ Remove: {display_name}", callback_data=safe_callback)])
        
        # Add a special button for direct removal with exact ID (handling the case when ID is 2502786837)
        if len(active_channels["destinations"]) == 1:
            # Special direct removal for known single destination
            first_dest = active_channels["destinations"][0]
            logger.info(f"Adding special direct removal button for {first_dest}")
            keyboard.append([InlineKeyboardButton(f"🗑️ Force Remove Channel", 
                             callback_data=f"force_remove_dest_{first_dest}")])
        
        # Add a special button for the known channel ID that exists in your configuration
        logger.info("Adding special direct removal button for 2502786837")
        keyboard.append([InlineKeyboardButton("🗑️ Remove Default Channel (2502786837)", 
                          callback_data="direct_remove_2502786837")])
    ...
    elif query.data == "direct_remove_2502786837":
        # This is a special direct handler specifically for the known channel ID 2502786837
        logger.info("Executing direct removal of channel 2502786837")
        
        # Check if we have this ID in the list (both as int and string forms)
        found = False
        removed_channel = None
        target_id = 2502786837
        
        # Try to find and remove as integer
        if target_id in active_channels["destinations"]:
            active_channels["destinations"].remove(target_id)
            found = True
            removed_channel = target_id
            logger.info(f"✓ Removed channel 2502786837 (as integer)")
        
        # Try to find and remove as string
        elif str(target_id) in active_channels["destinations"]:
            active_channels["destinations"].remove(str(target_id))
            found = True
            removed_channel = str(target_id)
            logger.info(f"✓ Removed channel 2502786837 (as string)")
        
        # If still not found, try index-based approach
        if not found:
            for idx, dest in enumerate(active_channels["destinations"]):
                if str(dest) == str(target_id):
                    removed_channel = active_channels["destinations"].pop(idx)
                    found = True
                    logger.info(f"✓ Removed channel 2502786837 (by index {idx})")
                    break
        
        # If still not found, as a last resort, completely reset the destinations list
        if not found:
            logger.warning("Could not find channel 2502786837 with regular methods, completely resetting destinations")
            previous_destinations = active_channels["destinations"].copy()
            active_channels["destinations"] = []
            found = True
            removed_channel = "all destinations"
            logger.info(f"✓ Reset all destinations because 2502786837 couldn't be found. Previous: {previous_destinations}")
        
        # Also clear the legacy destination field if it matches
        if active_channels["destination"] == target_id or str(active_channels["destination"]) == str(target_id):
            active_channels["destination"] = None
            logger.info("✓ Also cleared matching legacy destination field")
        
        # Save the updated configuration
        await save_config()
        
        # Clear message mappings
        # We need to make message_mapping available at the module level
        # so we can modify it in this function
        message_mapping.clear()  # Clear the existing dictionary instead of reassigning
        logger.info("Cleared message mapping cache")
        
        # Show success message
        if found:
            await edit_message_smartly(
                query.message,
                f"✅ Successfully removed channel 2502786837 from destinations.\n\n"
                f"A full reset of the message mapping cache was also performed to ensure clean state.\n\n"
                f"You can add new destination channels if needed.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="manage_destinations")]])
            )
        else:
            await edit_message_smartly(
                query.message,
                "❌ Failed to remove channel 2502786837.\n\n"
                "Please try using the 'Reset All Destinations' option as an alternative.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="manage_destinations")]])
            )
    
    elif query.data.startswith("remove_dest_") or query.data.startswith("remove_dest_idx_") or query.data.startswith("force_remove_dest_"):
        try:
            # Determine what type of callback data we're dealing with
            if query.data.startswith("remove_dest_idx_"):
                # This is our new index-based format
                parts = query.data.split("_", 4)  # Split into ['remove', 'dest', 'idx', INDEX, CHANNEL_ID]
                if len(parts) >= 4:
                    try:
                        idx = int(parts[3])
                        logger.info(f"Removing destination by index: {idx}")
                        if 0 <= idx < len(active_channels["destinations"]):
                            channel_id_str = str(active_channels["destinations"][idx])
                            logger.info(f"Found channel at index {idx}: {channel_id_str}")
                        else:
                            logger.warning(f"Index {idx} out of range for destinations list with length {len(active_channels['destinations'])}")
                            channel_id_str = parts[4] if len(parts) > 4 else "unknown"
                    except (ValueError, IndexError) as e:
                        logger.error(f"Error parsing index from callback data: {str(e)}")
                        channel_id_str = "unknown"
                else:
                    channel_id_str = "unknown"
            
            # Handle the force remove option
            elif query.data.startswith("force_remove_dest_"):
                prefix = "force_remove_dest_"
                channel_id_str = query.data[len(prefix):]
                logger.info(f"Force removing destination: {channel_id_str}")
                
                # With force removal, we know this is specifically channel 2502786837
                # Directly update the destinations list if we're sure about this
                if channel_id_str == "2502786837" or channel_id_str.strip() == "2502786837":
                    logger.info("Directly removing known channel ID from destinations")
                    if 2502786837 in active_channels["destinations"]:
                        active_channels["destinations"].remove(2502786837)
                        # Skip the rest of the complex logic by setting these values
                        found = True
                        removed_channel = 2502786837
                    
            # Normal removal by channel ID from callback data
            else:
                prefix = "remove_dest_"
                if query.data.startswith(prefix):
                    channel_id_str = query.data[len(prefix):]  # This gets everything after the prefix
                else:
                    # Fallback to split (should never happen, but added for safety)
                    channel_id_str = query.data.split("_", 2)[-1]
            
            # Print detailed debug information
            logger.info(f"=== DESTINATION REMOVAL DEBUG ===")
            logger.info(f"Original callback: {query.data}")
            logger.info(f"Extracted channel_id_str: '{channel_id_str}'")
            logger.info(f"Destinations (raw): {active_channels['destinations']}")
            logger.info(f"Destination types: {[type(d).__name__ for d in active_channels['destinations']]}")
            
            # COMPLETELY REBUILT DESTINATION REMOVAL LOGIC
            # =============================================
            
            # Make sure we have the original channel ID format
            original_channel_id_str = channel_id_str
            
            # SAFETY: Ensure destinations is always a list
            if not isinstance(active_channels["destinations"], list):
                logger.warning(f"Fixing destinations - not a list but {type(active_channels['destinations']).__name__}")
                if active_channels["destinations"] is None:
                    active_channels["destinations"] = []
                else:
                    active_channels["destinations"] = [active_channels["destinations"]]
            
            # Start with no matches found
            found = False
            removed_channel = None
            
            # Build detailed debug info
            debug_info = "DESTINATION CHANNEL REMOVAL DETAILS:\n\n"
            debug_info += f"Channel to remove: {channel_id_str}\n"
            debug_info += f"Current destinations: {[str(d) for d in active_channels['destinations']]}\n\n"
            
            # Handling the destination channel removal
            # ---- Approach #1: Try direct match as integer ----
            if channel_id_str.lstrip('-').isdigit():
                try:
                    channel_id_int = int(channel_id_str)
                    debug_info += f"Converted to integer: {channel_id_int}\n"
                    
                    # Check if this integer is directly in the list
                    if channel_id_int in active_channels["destinations"]:
                        active_channels["destinations"].remove(channel_id_int)
                        removed_channel = channel_id_int
                        found = True
                        debug_info += f"✓ Removed channel (direct integer match): {removed_channel}\n"
                except Exception as e:
                    debug_info += f"Integer conversion error: {str(e)}\n"
            
            # ---- Approach #2: Try exact string match ----
            if not found:
                # Try exact string match
                if channel_id_str in active_channels["destinations"]:
                    active_channels["destinations"].remove(channel_id_str)
                    removed_channel = channel_id_str
                    found = True
                    debug_info += f"✓ Removed channel (direct string match): {removed_channel}\n"
            
            # ---- Approach #3: Compare string representations of each destination ----
            if not found:
                # Try matching by string representation (handles type differences)
                match_idx = None
                for idx, dest in enumerate(active_channels["destinations"]):
                    if str(dest) == channel_id_str:
                        match_idx = idx
                        debug_info += f"✓ Found matching channel at index {idx}: {dest}\n"
                        break
                
                # If we found a match, remove it
                if match_idx is not None:
                    removed_channel = active_channels["destinations"].pop(match_idx)
                    found = True
                    debug_info += f"✓ Removed channel (string representation match): {removed_channel}\n"
            
            # ---- Approach #4: Try removing any potential encoding or spacing issues ----
            if not found:
                debug_info += "Trying more aggressive matching techniques...\n"
                
                # Clean the channel ID string (remove any whitespace, quotes, etc.)
                clean_channel_id = channel_id_str.strip().replace('"', '').replace("'", "")
                
                # Try to find a match with the cleaned ID
                match_idx = None
                for idx, dest in enumerate(active_channels["destinations"]):
                    dest_str = str(dest).strip().replace('"', '').replace("'", "")
                    debug_info += f"Comparing '{clean_channel_id}' with '{dest_str}'\n"
                    
                    if dest_str == clean_channel_id:
                        match_idx = idx
                        debug_info += f"✓ Found matching channel with cleaned string at index {idx}: {dest}\n"
                        break
                ...
                if match_idx is not None:
                    removed_channel = active_channels["destinations"].pop(match_idx)
                    found = True
                    debug_info += f"✓ Removed channel (cleaned string match): {removed_channel}\n"
            
            # ---- Approach #5: Last resort - check numeric part only ----
            if not found and channel_id_str.lstrip('-').isdigit():
                debug_info += "Trying numeric-only comparisons...\n"
                channel_id_int = int(channel_id_str)
                
                # Try to find any channel with the same numeric value regardless of type
                match_idx = None
                for idx, dest in enumerate(active_channels["destinations"]):
                    # Convert to string if needed, then extract numeric part
                    dest_str = str(dest)
                    if dest_str.lstrip('-').isdigit():
                        dest_int = int(dest_str)
                        debug_info += f"Comparing numeric value {channel_id_int} with {dest_int}\n"
                        
                        if dest_int == channel_id_int:
                            match_idx = idx
                            debug_info += f"✓ Found matching channel by numeric value at index {idx}: {dest}\n"
                            break
                ...
                if match_idx is not None:
                    removed_channel = active_channels["destinations"].pop(match_idx)
                    found = True
                    debug_info += f"✓ Removed channel (numeric value match): {removed_channel}\n"
            
            # If we found and removed the channel, also check the legacy destination
            if found and removed_channel is not None:
                # Also check and clear legacy destination if it matches
                destination = active_channels["destination"]
                if destination is not None:
                    # Compare as strings to handle type differences
                    if str(destination) == str(removed_channel):
                        active_channels["destination"] = None
                        debug_info += "✓ Also cleared the legacy destination field\n"
                
                # Save the updated configuration
                await save_config()
                
                # Re-initialize the message mapping cache since a destination was removed
                # This prevents the bot from trying to edit messages in channels that are no longer destinations
                message_mapping.clear()  # Clear the existing dictionary instead of reassigning
                logger.info("Cleared message mapping cache to prevent referencing removed destination")
                debug_info += "✓ Message mapping cache cleared to ensure clean state\n"
                
                # Show success message
                await edit_message_smartly(
                    query.message,
                    f"✅ Successfully removed channel from destinations.\n\n{debug_info if DEVELOPER_MODE else ''}",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="manage_destinations")]])
                )
            else:
                # Show failure message with debug info
                debug_info += "\nRaw callback data: " + query.data
                await edit_message_smartly(
                    query.message,
                    f"❌ Could not find the channel in destinations.\n\n{debug_info if DEVELOPER_MODE else ''}",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="manage_destinations")]])
                )
        except Exception as e:
            ...
            logger.error(f"Error removing destination channel: {error_type} - {error_msg}")
            ...
            if DEVELOPER_MODE:
                ...
                debug_info += f"- Callback Data: {query.data}\n"
                debug_info += f"- Raw channel ID: {channel_id_str if 'channel_id_str' in locals() else 'unknown'}\n"
                debug_info += f"- Current destinations: {[str(d) for d in active_channels['destinations']]}\n"
                debug_info += f"- Destination types: {[type(d).__name__ for d in active_channels['destinations']]}"
                
                # Include a snippet of the traceback (last 3 lines) for user
                tb_lines = tb_str.strip().split('\n')
                if len(tb_lines) > 3:
                    last_lines = '\n'.join(tb_lines[-3:])
                    debug_info += f"\n\nError Location:\n{last_lines}"
            
            # Show a more user-friendly message with detailed debug info for developers
            await edit_message_smartly(
                query.message,
                f"❌ Error removing destination channel.\n\n"
                f"Technical details: {error_type} - {error_msg}\n{debug_info if DEVELOPER_MODE else ''}\n\n"
                f"Please try again or contact support with the error details.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="manage_destinations")]])
            )
        ...
        if active_channels["destination"]:
            action_buttons.append([InlineKeyboardButton("🗑️ Remove Destination Channel", callback_data="remove_primary_destination")])
        ...
        # Handle deletion sync toggle
        current_status = "Enabled" if sync_deletions else "Disabled"
        text = f"🗑️ Deletion Synchronization: {current_status}\n\n"
        text += "When enabled, deleted messages in the source channel will also be deleted from destination channels.\n\n"
        text += "This requires that the bot stores message mappings in memory to track which messages correspond to each other."
        
        keyboard = [
            [InlineKeyboardButton(f"{'🔴 Disable' if sync_deletions else '🟢 Enable'} Deletion Sync", callback_data="toggle_deletion_sync")],
            [InlineKeyboardButton("◀️ Back to Menu", callback_data="back_to_menu")]
        ]
        
        await edit_message_smartly(query.message, text, reply_markup=InlineKeyboardMarkup(keyboard))
    
    elif query.data == "toggle_deletion_sync":
        # Toggle the sync_deletions flag
        sync_deletions = not sync_deletions
        
        # Save to config - no need to set BOT_CONFIG directly as save_reposting_state handles it
        await save_reposting_state()  # This already saves sync_deletions
        
        # Show confirmation
        status = "enabled" if sync_deletions else "disabled"
        text = f"✅ Deletion synchronization has been {status}.\n\n"
        text += f"Deleted messages in source channels {'will' if sync_deletions else 'will not'} be deleted from destination channels."
        
        keyboard = [[InlineKeyboardButton("◀️ Back", callback_data="deletion_sync")]]
        await edit_message_smartly(query.message, text, reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        keyboard = []
        for old_tag, new_tag in tag_replacements.items():
            # Ensure tag is properly encoded for the callback data
            safe_tag = str(old_tag).strip()
            safe_callback = f"remove_tag_{safe_tag}"
            logger.info(f"Creating tag removal callback data: {safe_callback}")
            keyboard.append([InlineKeyboardButton(f"🗑️ {old_tag} → {new_tag}", callback_data=safe_callback)])
        ...
        # Extract tag using a more reliable method
        prefix = "remove_tag_"
        old_tag = query.data[len(prefix):]  # Get everything after the prefix
        
        logger.info(f"Processing tag removal: {query.data}")
        logger.info(f"Extracted tag: '{old_tag}'")
        logger.info(f"Current tag replacements: {tag_replacements}")
        ...
        if old_tag in tag_replacements:
            ...
            found = True
            logger.info(f"✅ Found and removed tag via direct lookup: {old_tag} → {removed_value}")
        
        # If not found, try case-insensitive comparison
        if not found:
            for tag in list(tag_replacements.keys()):
                if tag.lower() == old_tag.lower():
                    removed_value = tag_replacements[tag]
                    del tag_replacements[tag]
                    found = True
                    logger.info(f"✅ Found and removed tag via case-insensitive matching: {tag} → {removed_value}")
                    old_tag = tag  # Use the original casing for display
                    break
                    
        # Save configuration if we found and removed a tag
        if found:
            ...
            # Detailed error with debugging info
            debug_info = f"\nTag: '{old_tag}'"
            debug_info += f"\nAvailable tags: {list(tag_replacements.keys())}"
            
            logger.warning(f"Tag replacement not found: {debug_info}")
            await query.edit_message_text(
    ...
    elif query.data == "remove_primary_destination":
        # Confirm before removing the primary destination
        if not active_channels["destination"]:
            await query.edit_message_text(
                "No primary destination channel is currently configured.",
        ...
        confirm_keyboard = [
            [
                InlineKeyboardButton("Yes, remove it", callback_data="confirm_remove_primary_destination"),
    ...
    elif query.data == "confirm_remove_primary_destination":
        # Remove the destination channel
        old_destination = active_channels["destination"]
        
        try:
            # Log detailed information before removal
            logger.info(f"Removing destination channel: {old_destination} (type: {type(old_destination).__name__})")
            logger.info(f"Current destinations list: {active_channels['destinations']}")
            
            # Remove from primary destination
            active_channels["destination"] = None
            
            # Also clear it from destinations list if it exists there
            if old_destination is not None:
                # Convert to string for comparison if needed
                old_dest_str = str(old_destination)
                # Convert all destinations to strings for comparison
                dest_strs = [str(d) for d in active_channels["destinations"]]
                
                if old_dest_str in dest_strs:
                    logger.info(f"Found {old_dest_str} in destinations list, removing it")
                    # Find the index to remove the matching item by its original type
                    idx = dest_strs.index(old_dest_str)
                    if idx >= 0 and idx < len(active_channels["destinations"]):
                        active_channels["destinations"].pop(idx)
                        logger.info(f"Removed destination at index {idx}")
                else:
                    logger.info(f"Destination {old_dest_str} not found in destinations list")
            
            # Save the updated configuration to persist to .env file
            await save_config()
            
            # Show success message with debugging info
            debug_info = ""
            if DEVELOPER_MODE:
                debug_info = f"\n\nDebug Info:\n- Old destination: {old_destination} (type: {type(old_destination).__name__})\n- Current destinations: {active_channels['destinations']}"
            
            await query.edit_message_text(
                f"✅ Destination channel has been removed successfully.{debug_info}\n\n"
                f"You will need to set a new destination channel to use the reposting functionality.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back to Menu", callback_data="back_to_menu")]])
            )
        except Exception as e:
            # Provide detailed error information
            error_msg = str(e)
            logger.error(f"Error removing destination channel: {error_msg}")
            ...
            await query.edit_message_text(
                f"❌ Error removing destination channel.{debug_info}\n\n"
        ...
        try:
            ...
            if purge_and_leave:
                text += f"The bot will delete messages, show credits, post a GIF, send audio, and leave the channel.\n\n"
                ...
                try:
                    ...
                    if purge_and_leave:
                        ...
                        try:
                            ...
                            # Prepare credits message with hyperlink
                            credit_message = f"Bot by <a href='{CREATOR_LINK}'>{CREATOR_NAME}</a>"
                            credit_sent = False
                            
                            # We'll send the credits as part of the GIF if leaving
                            if purge_and_leave:
                                # Skip standalone credits message - it will be included with GIF
                                credit_sent = True
                                logger.info("Skipping standalone credits - will include with GIF")
                            else:
                                # For regular purge (not leaving), send credits as a message
                                try:
                            ...
                            if purge_and_leave:
                                ...
                                if FAREWELL_GIFS:
                                    try:
                                        # Select a random GIF from our globally imported random module
                                        random_gif = random.choice(FAREWELL_GIFS)
                                        logger.info(f"Selected farewell GIF: {random_gif}")
                                        
                                        # Print debug info
                                        logger.info(f"Attempting to send GIF from URL: {random_gif}")
                                        
                                        # Check if it's a URL or local file
                                        if random_gif.startswith(('http://', 'https://')):
                                            # Try sending message with the GIF as external URL using Telegram's built-in GIF support
                                            # Include credits as caption
                                            try:
                                                await user_client.send_message(
                                                    channel_entity,
                                                    credit_message,  # Use credits as caption
                                                    file=random_gif,  # URL as file
                                                    parse_mode='html'  # Enable HTML for the hyperlink
                                                )
                                                logger.info("Sent farewell GIF with credits caption as URL")
                                                gif_sent = True
                                            except Exception as url_err:
                                                logger.error(f"Failed to send GIF as URL: {str(url_err)}")
                                                
                                                # Alternative: try sending with a different method
                                                try:
                                                    await user_client.send_file(
                                                        channel_entity,
                                                        random_gif,  # URL as file
                                                        caption=credit_message,  # Credits as caption
                                                        parse_mode='html'  # Enable HTML for the hyperlink
                                                    )
                                                    logger.info("Sent GIF with credits using send_file")
                                                    gif_sent = True
                                                except Exception as file_err:
                                                    logger.error(f"Failed to send GIF with send_file: {str(file_err)}")
                                                    
                                                    # Last resort: try sending the URL as a text message with credits
                                                    try:
                                                        await user_client.send_message(
                                                            channel_entity,
                                                            f"{credit_message}\n\n{random_gif}"  # Credits + URL
                                                        )
                                                        logger.info("Sent GIF URL with credits as text message")
                                                        gif_sent = True
                                                    except Exception as txt_err:
                                                        logger.error(f"Failed to send GIF URL as text: {str(txt_err)}")
                                        else:
                                            # Local file path
                                            try:
                                                await user_client.send_file(
                                                    channel_entity,
                                                    random_gif,  # Local file path
                                                    caption=credit_message,  # Credits as caption
                                                    parse_mode='html'  # Enable HTML for the hyperlink
                                                )
                                                logger.info("Sent farewell GIF from local file with credits caption")
                                                gif_sent = True
                                            except Exception as local_err:
                                                logger.error(f"Failed to send local GIF with caption: {str(local_err)}")
                                                # Fallback: send without HTML formatting
                                                try:
                                                    await user_client.send_file(
                                                        channel_entity,
                                                        random_gif,  # Local file path
                                                        caption=f"Bot by {CREATOR_NAME} ({CREATOR_USERNAME})"  # Plain text caption
                                                    )
                                                    logger.info("Sent farewell GIF with plain text credits")
                                                    gif_sent = True
                                                except Exception as plain_err:
                                                    logger.error(f"Failed to send GIF with plain caption: {str(plain_err)}")
                                                    # Last resort: just send the GIF
                                                    try:
                                                        await user_client.send_file(
                                                            channel_entity,
                                                            random_gif  # Just the GIF, no caption
                                                        )
                                                        logger.info("Sent farewell GIF without caption (fallback)")
                                                        gif_sent = True
                                                    except Exception as no_caption_err:
                                                        logger.error(f"Failed to send GIF without caption: {str(no_caption_err)}")
                                        
                                        # Add small delay to ensure correct ordering
                                        await asyncio.sleep(1)
                                ...
                                # 3. Send the audio file (even if GIF failed)
                                audio_sent = False
                                
                                # Simple, direct audio file selection
                                try:
                                    # Explicitly define audio files here for maximum reliability
                                    audio_files = [
                                        "assets/farewell/laugh_or_cry.m4a",  # First audio file (simplified name)
                                        "assets/farewell/piercing_light.mp3"  # Second audio file (simplified name)
                                    ]
                                    ...
                                    for file in audio_files:
                                        if os.path.exists(file):
                                            ...
                                            logger.info(f"Audio file verified: {file}")
                                        else:
                                            logger.warning(f"Audio file missing: {file}")
                                    
                                    # Debug output for all available files
                                    logger.info(f"Total available audio files: {len(available_files)}")
                                    
                                    if len(available_files) > 0:
                                        # Force deterministic choice based on current time 
                                        # (will alternate between files)
                                        current_second = datetime.datetime.now().second
                                        chosen_index = current_second % len(available_files)
                                        audio_path = available_files[chosen_index]
                                        
                                        logger.info(f"TIME-BASED SELECTION - Second: {current_second}, Index: {chosen_index}")
                                        logger.info(f"SELECTED AUDIO: {audio_path}")
                                    else:
                                        # Emergency fallback if no files exist
                                        audio_path = "assets/farewell/bye.mp3"  # Default minimal audio
                                        logger.warning(f"No audio files available, using emergency fallback: {audio_path}")
                                except Exception as audio_err:
                                    # Last resort fallback
                                    audio_path = "assets/farewell/bye.mp3"
                                    logger.error(f"Critical audio selection error: {str(audio_err)}")
                                    logger.error(f"Using emergency fallback audio: {audio_path}")
                                
                                # Add a small delay regardless of whether GIF was sent
                                await asyncio.sleep(1)
                                
                                # Check if audio file exists
                                if os.path.exists(audio_path):
                                    logger.info(f"Found audio file at path: {audio_path}")
                                    try:
                                        # Try different ways to send the audio
                                        try:
                                            # First attempt: Send as regular audio file
                                            await user_client.send_file(
                                                channel_entity,
                                                audio_path,
                                                voice_note=False,
                                                attributes=[DocumentAttributeAudio(
                                                    ...
                                                    title="Farewell Audio",
                                                    performer="Bot"
                                                )]
                                            )
                                            logger.info("Sent farewell audio successfully")
                                            audio_sent = True
                                        except Exception as attr_err:
                                            logger.error(f"Failed to send audio with attributes: {str(attr_err)}")
                                            
                                            # Second attempt: Send without attributes
                                            try:
                                                await user_client.send_file(
                                                    channel_entity,
                                                    audio_path,
                                                ...
                                                logger.info("Sent farewell audio without attributes")
                                                audio_sent = True
                                            except Exception as simple_err:
                                                logger.error(f"Failed to send simple audio: {str(simple_err)}")
                                                
                                                # Third attempt: Try as voice note
                                                try:
                                                    await user_client.send_file(
                                                        channel_entity,
                                                        audio_path,
                                                    ...
                                                    logger.info("Sent farewell audio as voice note")
                                                    audio_sent = True
                                                except Exception as voice_err:
                                                    logger.error(f"Failed to send as voice note: {str(voice_err)}")
                                    except Exception as audio_err:
                                        logger.error(f"Failed to send audio: {str(audio_err)}")
                                else:
                                    logger.warning(f"Audio file not found at path: {audio_path}")
                                    
                                    # Try to list what files ARE in the assets directory
                                    try:
                                        import glob
                                        audio_files = glob.glob("assets/farewell/*.*")
                                        logger.info(f"Files found in farewell directory: {audio_files}")
                                    except Exception as list_err:
                                        logger.error(f"Error listing audio files: {str(list_err)}")
                                
                                # 4. Now prepare to leave the channel
                                try:
                                    # Use a different message each time to avoid "message not modified" error
                                    await status_msg.edit_text(
                                        f"👋 Leaving channel... ({random.randint(1000, 9999)})"
                                    )
                                except Exception as msg_err:
                                    logger.error(f"Status message update error: {str(msg_err)}")
                            else:
                            ...
                            # Leave the channel
                            try:
                                # Use a timestamp to make each message unique
                                await status_msg.edit_text(
                                    f"👋 Leaving channel... ({random.randint(1000, 9999)})"
                                )
                            except Exception as msg_err:
                                logger.error(f"Update error when preparing to leave: {str(msg_err)}")
    ...
    elif query.data == "reset_all_channels":
        # First, ask for confirmation with dramatic warning
        confirm_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("☢️ EXECUTE NUCL3AR WIPE ☢️", callback_data="confirm_reset_all")],
            [InlineKeyboardButton("❌ ABORT! ABORT!", callback_data="back_to_menu")]
        ])
        
        warning_message = (
            "☢️ Nucl3ar Option Activated ☢️\n\n"
            "⚠️ EXTREME CAUTION REQUIRED ⚠️\n\n"
            "This will perform a complete and irreversible nuclear wipe of your configuration:\n"
            "• ALL source channels will be obliterated\n"
            "• ALL destination channels will be purged\n"
            "• ALL message mappings will be destroyed\n\n"
            "Use this option ONLY when everything is broken and you need a completely fresh start.\n\n"
            "⚠️ THIS OPERATION CANNOT BE UNDONE! ⚠️\n\n"
            "Confirm execution of Nucl3ar protocol?"
        )
        ...
        # This is the actual reset handler that will clear everything
        logger.info("Executing complete channel reset")
        
        # Clear source channels
        source_count = len(active_channels["source"]) if active_channels["source"] else 0
        active_channels["source"] = []
        
        # Clear destinations - all forms
        dest_count = 0
        if active_channels["destination"]:
            dest_count += 1
            active_channels["destination"] = None
        ...
        if active_channels["destinations"]:
            dest_count += len(active_channels["destinations"])
            active_channels["destinations"] = []
        
        # Clear message mappings
        message_mapping.clear()
        if 'message_mapping_order' in globals():
            message_mapping_order.clear()
        
        # Save the updated configuration
        await save_config()
        
        # Prepare nuclear wipe success message with dramatic details
        result_message = (
            "☢️ NUCL3AR WIPE COMPLETED ☢️\n\n"
            f"⚠️ {source_count} source channel(s) obliterated\n"
            f"⚠️ {dest_count} destination channel(s) purged\n"
            "⚠️ All message mappings destroyed\n\n"
            "The nuclear protocol has been successfully executed.\n"
            "All systems have been reset to ground zero.\n\n"
            "Your configuration has been completely wiped clean.\n"
            "You may now rebuild from the ashes."
        )
        ...
        logger.info(f"Reset completed: {source_count} source and {dest_count} destination channels removed")
        ...
        channel_buttons = [
            ...
            [
                ...
                InlineKeyboardButton("🗑️ Remove Destination", callback_data="remove_primary_destination")
            ],
            [
                InlineKeyboardButton("⚙️ Channel Management", callback_data="channel_settings_menu")
            ]
...
async def start_bot():
    ...
    try:
        ...
        logger.info(f"Memory optimization active: {MAX_RECENT_MESSAGES} messages, {MAX_CACHE_AGE_HOURS}h expiration")
        logger.info(f"Cache cleanup interval: Every {CACHE_CLEANUP_INTERVAL} operations")
        
        # Run the bot until stopped
        await bot.updater.start_polling()
        
        # Create a background task for periodic cache cleanup
        async def periodic_cache_cleanup():
            ...
            while True:
                try:
                    ...
                    cleanup_counter += 1
                    removed = await cleanup_message_cache()
                    logger.info(f"Hourly cache cleanup (#{cleanup_counter}): Removed {removed} entries")
                    logger.info(f"Memory stats: Active mappings={len(message_mapping)}, "
                               f"Hits={memory_stats['cache_hits']}, "
                               f"Misses={memory_stats['cache_misses']}")
                except Exception as e:
