...
try:
    # Format: {"source_channels": [channel_id1, channel_id2, ...], "destination_channel": channel_id, "destination_channels": [channel_id1, channel_id2, ...]}
    default_config = '{"source_channels": [], "destination_channel": null, "destination_channels": []}'
    CHANNEL_CONFIG = json.loads(os.getenv("CHANNEL_CONFIG", default_config))
    
    # Ensure all required keys exist
    if "source_channels" not in CHANNEL_CONFIG:
        CHANNEL_CONFIG["source_channels"] = []
    if "destination_channel" not in CHANNEL_CONFIG:
        CHANNEL_CONFIG["destination_channel"] = None
    if "destination_channels" not in CHANNEL_CONFIG:
    ...
    CHANNEL_CONFIG = {"source_channels": [], "destination_channel": None, "destination_channels": []}
...
def save_bot_config():
    ...
    # Import the developer mode setting if it exists in the bot module
    try:
        import bot
        if hasattr(bot, 'DEVELOPER_MODE'):
            BOT_CONFIG["developer_mode"] = bot.DEVELOPER_MODE
            logger.info(f"Including developer_mode={bot.DEVELOPER_MODE} in bot configuration")
    except ImportError:
        logger.info("Could not import bot module to get DEVELOPER_MODE")
    
    # Make sure developer_mode is included in the config
    if "developer_mode" not in BOT_CONFIG:
        BOT_CONFIG["developer_mode"] = True  # Default to True for better debugging
    
    config_json = json.dumps(BOT_CONFIG)
    os.environ["BOT_CONFIG"] = config_json
    
    logger.info(f"Saving bot configuration with keys: {list(BOT_CONFIG.keys())}")
    ...
    try:
        ...
        logger.info(f"✅ Successfully updated bot configuration in .env file")
    except Exception as e:
        error_msg = str(e)
        logger.error(f"❌ Failed to update .env file with bot configuration: {error_msg}")
