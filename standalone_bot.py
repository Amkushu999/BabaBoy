...
async def start_bot():
    ...
    try:
        ...
        # Log information about memory optimization configuration
        logger.info(f"Memory optimization: MAX_RECENT_MESSAGES={bot.MAX_RECENT_MESSAGES}, MAX_CACHE_AGE_HOURS={bot.MAX_CACHE_AGE_HOURS}")
        logger.info(f"Cache cleanup interval: Every {bot.CACHE_CLEANUP_INTERVAL} operations")
                ...
                if (hasattr(bot, 'API_ID') and bot.API_ID and 
                    ...
                    if await bot.user_client.is_user_authorized():
                        ...
                        if bot.active_channels["source"]:
                            ...
                            # Do the same for edited messages handler - with enhanced logging
                            for builder in builders:
                            ...
                            # Check if sync_deletions is enabled
                            if hasattr(bot, 'get_sync_deletions'):
                                sync_status = bot.get_sync_deletions()
                                logger.info(f"Deletion sync status: {'ENABLED' if sync_status else 'DISABLED'}")
                    else:
